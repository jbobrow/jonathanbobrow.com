---
title: "Future Energy"
slug: "future-energy"
order: 25
featured: false
role: "Designer & Engineer · Potion"
year: "2013"
heroImage: "/images/projects/future-energy/hero.jpg"
heroAlt: "Museum of Science and Industry Future Energy interactive exhibit"
---

Museum of Science and Industry - Future Energy

With Potion's team of designers, developers, producers, and 3D artists, I helped to create an immersive space to learn about the future of energy. I was involved in the project from concept pitch to installation, paper prototype to design and software development.

When thinking about energy consumption, one of the biggest hurdles is imagining the actual amounts of energy when you cannot see it in any measurable form. We decided to make the energy unit the star of our installation, with issues such as environment and finances coming in to play as dependent variables. Up to 30 kids or adults can participate at once, grouped into 5 teams, competing to conserve as much energy as possible.

There are 5 interactive installations, each with different form factors that make the space feel more like an augmented reality than a museum interactive screen. I wrote projection mapping software and planned for projectors to provide both a user interface as well as optical illusion to give the impression that a physical car or city is changing in front of your eyes.

I was most directly focused on two games: one a physical model of a house that allows the user to touch any item to learn its energy impact, and the other a simulation of a neighborhood where creating stacked mixed-use buildings reduces the amount of travel time and energy consumed. The interface shows the user which people are headed where, creating a fun and quite difficult optimization puzzle.
